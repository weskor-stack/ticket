                                ...
                                <tbody>
                                    @foreach ($statuses as $status)
                                        <tr style="text-align: center; font-size: 15px;  font-weight: bold; text-align: center; vertical-align: center;">
                                            ...
                                            <td>
                                                <form action="{{ route('statuses.destroy',$status->status_id) }}" method="POST">
                                                    <a class="btn btn-sm btn-primary " href="{{ route('statuses.show',$status->status_id) }}"><i class="fa fa-fw fa-eye"></i> Ver</a>
                                                    <a class="btn btn-sm btn-success" href="{{ route('statuses.edit',$status->status_id) }}"><i class="fa fa-fw fa-edit"></i> Editar</a>
                                                    @csrf
                                                    @method('DELETE')
