<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

class CreateTicketCustomerTable extends Migration
{
    /**
     * Run the migrations.
     *
     * @return void
     */
    public function up()
    {
        Schema::create('ticket_customer', function (Blueprint $table) {
            $table->unsignedInteger('ticket_id')->index('fk_ticket_customer_ticket1_idx');
            $table->unsignedInteger('customer_id')->index('idx_customer');
            $table->unsignedInteger('status_id')->index('fk_ticket_customer_status1_idx');
            $table->unsignedInteger('user_id')->index('idx_user');
            $table->timestamp('date_registration')->useCurrent();

            $table->primary(['ticket_id', 'customer_id']);
        });
    }

    /**
     * Reverse the migrations.
     *
     * @return void
     */
    public function down()
    {
        Schema::dropIfExists('ticket_customer');
    }
}
