...
class AddForeignKeysToOrderEmployeeScheduleTable extends Migration
{
    ...
    public function up()
    {
        Schema::table('order_employee_schedule', function (Blueprint $table) {
            $table->foreign(['order_service_id', 'employee_id'], 'fk_order_employee_schedule_order_employee1')->references(['order_service_id', 'employee_id'])->on('order_employee')->onUpdate('NO ACTION')->onDelete('NO ACTION');
        });
    }
